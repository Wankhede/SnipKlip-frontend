...
import { getPaymentReport } from 'services/reports';
  ...
  useEffect(() => {
    if (income && cos) {
      const fetchData = async () => {
        try {
          const response = await getPaymentReport({ slot: slot });
          const { data } = response.data;
          setOptions({
            ...options,
            xaxis: {
              ...options.xaxis,
              categories: data.mode_of_payments,
            },
          });
          setSeries([
            {
              name: 'Payment Mode',
              data: data.mode_of_payments_count
            }
          ]);
          // setTotal(data.total);
        } catch (error) {
