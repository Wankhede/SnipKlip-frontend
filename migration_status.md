...
## Upstream re-sync (2026-07-23)

| Item | Result |
|---|---|
| Source branch selected | `upstream/main` (tip `03b21f9`, merged from `dev` on 2024-11-15) — newest commits across all remote branches |
| Delta vs local | None — local `dev` already contains full upstream ancestry plus SnipKlip adaptations |
| Branding cleanup | Removed `hexa.png`; footer/welcome/maintenance alt text use SnipKlip |
| Port alignment | Defaults and scripts use backend `:8082`, frontend `:8083` |
| Typecheck | `tsc --noEmit` PASS |
| Production build | `npm run build` PASS |
| Dev smoke | `GET /login` and `/register` → HTTP 200 with SnipKlip meta |
