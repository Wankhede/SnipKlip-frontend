...
const NavCollapse = ({ menu, level, parentId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) => {
  ...
  const navCollapse = menu.children?.map((item) => {
    ...
    // Check if item.accessKey is in the array of "WEB_HEADER" keys
    const hasAccess = isSuperAdmin ? true : (item.accessKey ? (keys['WEB_HEADER'] || []).includes(item.accessKey) : true);
    if (hasAccess) {
      switch (item.type) {
        case 'collapse':
          return (
