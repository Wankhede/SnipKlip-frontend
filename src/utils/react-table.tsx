...
export function DefaultColumnFilter({ column: { filterValue, Header, setFilter, id }, column, setAllFilters, setGlobalFilter }: any) {
    const intl = useIntl();
    ...
    return (
        <TextField
            ...
            placeholder={typeof column.Header === 'string'
                ? column.Header
                : typeof column.Header === 'object'
                    ? intl.formatMessage({ id: column.Header.props.id })
...
export function SelectColumnFilter({ column: { filterValue, setFilter, preFilteredRows, id, options } }: { column: FilterColumnProps }) {
