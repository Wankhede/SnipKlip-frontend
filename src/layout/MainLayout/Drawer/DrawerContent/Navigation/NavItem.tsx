  ...
  let listItemProps: {
    ...
    component: forwardRef((props, ref) => (
      <NextLink href={item.url!} passHref>
        <Link {...props} target={itemTarget} onClick={() => { localStorage.setItem('current_page', item.accessKey!) }} />
      </NextLink>
    ))
  };
  if (item?.external) {
      ...
      {menuOrientation === LAYOUT_CONST.VERTICAL_LAYOUT || downLG ? (
        ...
        >
          ...
          {(drawerOpen || (!drawerOpen && level !== 1)) && (
            <ListItemText
              primary={
                <Typography variant="h6" sx={{ color: isSelected ? iconSelectedColor : textColor }} whiteSpace='pre-wrap'>
                  {item.title}
                </Typography>
              }
            />
