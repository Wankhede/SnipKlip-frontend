...
const NavGroup = ({ item, lastItem, remItems, lastItemId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) => {
  ...
  const navCollapse = item.children?.map((menuItem, index) => {
    // Retrieve the keys associated with the "WEB_HEADER" tag from localStorage
    const keysJSON = localStorage.getItem('accessKey');
    const group = localStorage.getItem('group');
    const keys = keysJSON ? JSON.parse(keysJSON) : {};
    const isSuperAdmin = group === "Admin" ? true : false;
    // Check if menuItem.accessKey is in the array of "WEB_HEADER" keys
    const hasAccess = isSuperAdmin ? true : (menuItem.accessKey ? (keys['WEB_HEADER'] || []).includes(menuItem.accessKey) : true);
    if (hasAccess) {
