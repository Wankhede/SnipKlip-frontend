...
export const TablePagination = ({ gotoPage, rows, setPageSize, pageSize, pageIndex, ...other }: TablePaginationProps) => {
  ...
  return (
    <Grid container alignItems="center" justifyContent="space-between" sx={{ width: 'auto' }}>
      ...
      <Grid item sx={{ mt: { xs: 2, sm: 0 } }}>
        <Pagination
          count={Math.ceil((other.pageCount ? other.pageCount : rows.length) / pageSize)}
          page={pageIndex + 1}
        ...
        {allColumns.map((column: ColumnInstance) => {
          ...
          return (
            ...
            >
              ...
              />
              <ListItemText primary={typeof column.Header === 'string' ? column.Header :  typeof column.Header === 'object' ? column.Header : typeof column.Header === 'function' ? intl.formatMessage({id: column.id}) : column?.title} />
            </MenuItem>
          );
        })}
      </Select>
...
export const CSVExport = ({ data, filename, headers }: CSVExportProps) => {
  return (
    <CSVLink data={data} filename={filename} onClick={(event:any) => {
      if (data.length === 0) {
        EssentialMethods.showSnackbar("No Record Selected", errorColor)
        return false;
      }else{
        return true;
      }
